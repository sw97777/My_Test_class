...
#Create Blank Tables
stats<-data.frame(Firm_be_analyzed=character(),Date=character(), Analyst_Company=character(),Opinions=character(), Price=character())
...
for(i in 1:nrow(all_firms)){
  tryCatch({
    html<- read_html(theurl[1])
    opinions<-html_node(html,"#analyst_opinions.graviton.sticky-breakpoint-height")
    opinions_table<-html_table(opinions)
    new_opinions_table<-opinions_table[,c(1,3,5,6)]
    ...
    new_opinions_table<-new_opinions_table[,c(5,1,2,3,4)]
...
write.table(stats,file="S&P_500_Firms_Stock_Analysis.csv",sep=",")
